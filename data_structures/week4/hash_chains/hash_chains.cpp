...
public:
    ...
    void processQuery2(const Query& query){
        if ( query.type == "check"){
            const list<string>& L = hash_table[query.ind];
            for(list<string> :: const_iterator iter = L.begin(); iter != L.end(); ++iter)
                cout<< *iter << " ";
            cout<<endl;
        }else if (query.type == "find"){
            const list<string>& L = hash_table[hash_func(query.s)];
            list<string>::const_iterator iter = find(L.begin(), L.end(), query.s);
            writeSearchResult(iter != L.end());
        }else if (query.type == "add"){
            list<string>& L = hash_table[hash_func(query.s)];
            list<string>::iterator iter = find(L.begin(), L.end(), query.s);
            if (iter == L.end()) 
